...
``papers/claude_design.md``. ALL numbers are the locked experimental values —
nothing is invented, rounded "in our favour", or inflated. The honest framing
(AR is the fastest method; "2.22×" is relative to vanilla speculative decoding;
joint ≈ cascade is a statistical tie; 7B/1.5B is a null) is preserved exactly,
including the dedicated "honest picture" slide.
...
def slide_theory(prs):
    ...
    cards = [
        ...
        ("B", "Беллман-инвариантность аддитивного штрафа качества", False),
...
def slide_results_main(prs):
    s = new_slide(prs)
    header(s, "Результаты: пара 14B / 0.5B", "GSM8K · n = 1500 · McNemar", 9)
    ...
    run(p, "+4,07 п.п.", size=54, color=GREEN, bold=True)
    p2 = tf.add_paragraph(); p2.alignment = PP_ALIGN.CENTER
    run(p2, "EM к прямой генерации (p = 0,0205)", size=16, color=GRAPHITE)
    ...
    notes(s, "Адаптивное управление повышает точность на +4 пункта при значимости p=0,02 и восстанавливает "
             "пропускную способность до 2,2× от ванильного спекулятивного декодирования.")


def slide_ablation(prs):
    s = new_slide(prs)
    header(s, "Адаптивность нетривиальна: теорема E", "Сравнение с фиксированным порогом · n = 300", 10)
    ...
    notes(s, "Нужна ли адаптивность, если порог можно подобрать? Совместная политика доминирует любой "
             "фиксированный порог сразу по точности и по скорости.")
...
def slide_defense(prs):
    ...
    items = [
        ...
        "Доказаны выборочная сложность (A), Беллман-инвариантность штрафа (B), оценка и точный разрыв совместной и каскадной политик (C, D), слабое доминирование (2.3) и Парето-скаляризация (2.4).",
        "Адаптивное управление значимо превосходит все неадаптивные базисы: +4,07 п.п. EM на 14B/0.5B (p = 0,0205) и +4…+8 п.п. над фиксированным порогом при ≈ 3,7× скорости.",
        "Установлены границы применимости: метод сводится к каскадной политике (теорема D) и не даёт выигрыша при низком отношении мощностей моделей.",
    ]
    ...
    notes(s, "На защиту выносятся четыре положения: формализуемость совместного управления как конечного MDP; "
             "доказанные теоретические свойства; значимое превосходство над неадаптивными базисами; границы применимости.")
...
def slide_thanks(prs):
    ...
    run(p5, "github.com/levvius/adaptive-speculative-decoding", size=16, color=TURQUOISE, bold=True, name=MONO)
    ...
    notes(s, "Весь код, тесты, манифесты и полный текст работы открыты — QR ведёт в репозиторий. "
