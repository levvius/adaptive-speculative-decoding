# -*- coding: utf-8 -*-
"""
Сборка первой редакции магистерской ВКР Козина А.А. в papers/ИС61_fpm_КозинАА_2026.docx.
...
Все числовые результаты взяты из docs/RESULTS.md
...
OUT = os.path.join(HERE, "ИС61_fpm_КозинАА_2026.docx")
...
TITLE = block([
    ...
    P("Работу выполнил А.А. Козин", indent=False, jc="both", space_after=120),
    ...
    P("Научный руководитель Калайдина Г.В.", indent=False, jc="both", space_after=600),
...
REFERAT = block([
    ...
    P("Основные результаты. Предложен метод JointAdaSpec, формализующий совместное "
      ...
      "управления. После аудита перед защитой крупный прогон на паре Qwen2.5-14B-Instruct "
      "→ Qwen2.5-0.5B-Instruct трактуется как artifact-backed thesis-snapshot evidence: он показывает "
      "потенциал адаптивного управления (+4,07 п.п. exact match, p = 0,0203, n = 1500; "
      "10,84 ток/с, что в 2,22 раза выше ванильного спекулятивного декодирования), но "
      "новые post-defense block-v1 claims требуют повторного прогона с новой 9-action "
      "политикой. Доказано, что совместная и каскадная политики статистически "
...
SODER = block([
    ...
    TOC_LINE("2.2.2 исправленная reward-shaping формулировка (теорема B)", 37, level=2),
    ...
    TOC_LINE("4.5 Адаптивность против фиксированного порога (эксперимент E)", 60, level=1),
...
VVEDENIE = block([
    ...
    P("Научная новизна работы состоит в том, что длина черновика и порог нечёткой "
      "верификации рассматриваются как связанные переменные управления в одной из "
      "первых табличных MDP-постановок такого типа, а не как независимо настраиваемые "
      "гиперпараметры. Эта постановка обладает "
      "малым пространством состояний и допускает точное решение методом value "
      ...
      "корректный анализ reward shaping, линейная по мере "
...
RAZDEL2 = block([
    ...
    P("Действие на каждом шаге выбирается из пространства версии 2: либо продолжить "
      "накопление чернового блока, либо верифицировать весь накопленный блок с одним из "
      "порогов T. Старое имя stop сохраняется только как compatibility alias входных артефактов "
      "и не сериализуется в новых политиках. Порог T из семейства нечёткой верификации "
      "(1.4) дискретизуется по геометрической сетке"),
    ...
    P("Геометрическая сетка выбрана потому, что влияние порога на вероятность принятия "
      ...
      "пространство действий состоит из одного действия продолжения и действий верификации"),
    EQ(mrow("A", op(" = "), "{continue}", op(" ∪ "), "{verify@T : T ∈ ",
            msub("A", "verif"), "}", op(", "), mdelim("A", "|", "|"), op(" = "),
            "1", op(" + "), "8", op(" = "), "9"), "2.6"),
    P("Размер |A| = 9 достаточен для разрешения совместной политики при стоимости одной "
    ...
    H3("2.2.2 исправленная reward-shaping формулировка (теорема B)"),
    P("Конструкция награды (2.8) вводит штраф качества как слагаемое, зависящее от "
      ...
      "оператора Беллмана."),
    PM(["Теорема B (исправленная reward-shaping формулировка). Пусть к награде добавлен штраф вида ",
...
def razdel3(fig3_block):
    ...
    P("Качество кода поддерживается непрерывной интеграцией. При каждом изменении система "
      ...
      "pytest-набор (на defense snapshot: 94 теста в 19 файлах каталога tests/), дымовой прогон SpecExec и "
...
def razdel4(figs):
    ...
    P("Настоящий раздел представляет экспериментальную оценку метода JointAdaSpec. Подраздел "
      ...
      "4.5–4.6 проверяют адаптивность против фиксированного порога (эксперимент E) и поведение "
    ...
    H2("4.3 Thesis snapshot results: пара 14B/0.5B"),
    P("На основной паре сравнивались четыре метода: прямая генерация целевой моделью "
      ...
      "таблице 4.2 как artifact-backed thesis-snapshot evidence; новые post-defense block-v1 "
      "claims относятся к будущим rerun в ветке main."),
    TBL_CAPTION("Таблица 4.2 — Thesis snapshot results на паре 14B/0.5B (GSM8K, n = 1500)"),
    ...
    P("В артефактах thesis snapshot совместная политика повышает точность на +4,07 п.п. "
      "относительно прямой генерации (p = 0,0203, 95 %-й доверительный интервал "
      "[+0,73; +7,40]) при пропускной способности 10,84 ток/с — в 2,22 раза выше "
      "ванильного спекулятивного декодирования. Этот результат используется как "
      "artifact-backed thesis evidence для snapshot ВКР; новые block-v1 claims выносятся в main. "
      "Совместная и каскадная политики статистически неразличимы: их разность "
      ...
      "не как «совместная лучше всех», а как «MDP-постановка, теория, исправленная block-v1 "
      "реализация и воспроизводимый план rerun отделены от thesis-snapshot чисел»."),
    P("Парный характер критерия Макнемара здесь существен. Значимость определяют не валовые "
      ...
      "расходятся. В артефактах thesis snapshot этот перевес давал p < 0,05 и доверительный "
      "интервал [+0,73; +7,40], а после аудита этот вывод трактуется как "
      "artifact-backed thesis-snapshot evidence; future block-v1 benchmark claims требуют fresh rerun в main."),
    figs["pareto"],
    ...
    H2("4.5 Адаптивность против фиксированного порога (эксперимент E)"),
    ...
    P("Результаты следует сопроводить обсуждением угроз валидности. Измерения скорости "
      ...
      "только по GSM8K: бенчмарки кода и диалога (в том числе MT-Bench [19]) в текущем "
      "раннере не оцениваются по точному совпадению, поэтому выводы о качестве ограничены "
      "областью математических рассуждений."),
    ...
    P("Совокупность результатов складывается в следующий честный вывод. Вклад работы — это "
      ...
      "вместе с теоретическим аппаратом (теоремы A, B, C, D, 2.3, 2.4), исправленная "
      "block-v1 реализация и воспроизводимый rerun protocol, а не утверждение "
      "«совместная политика побеждает все методы». Thesis-snapshot прогоны показывают потенциал "
      "адаптивного управления относительно ванильного SD и фиксированного нечёткого порога, "
      "но новые post-defense block-v1 claims требуют повторного прогона. Совместная политика "
      "совпадает с другой адаптивной MDP-политикой, каскадной, ровно как предсказывает теорема D. "
...
ZAKL = block([
    ...
    P("Предложенный метод JointAdaSpec формализует выбор пары (длина черновика, порог "
      ...
      "точно методом итерации по ценности без обучения нейросетевых модулей. Для пары "
      "Qwen2.5-14B → 0.5B с высоким отношением мощностей зафиксированный прогон показывает потенциал "
      "метода: +4,07 п.п. exact match относительно прямой генерации (p = 0,0203 по парному "
      "критерию Макнемара при n = 1500) и восстановление пропускной способности до 2,22-кратной "
      "относительно ванильного спекулятивного декодирования. Этот результат отделён от "
      "нового post-defense block-v1 claim, который требует повторного прогона с новой политикой."),
    P("Центральный теоретический и эмпирический результат — точная характеризация "
      ...
      "исправленная reward-shaping формулировка (теорема B), линейная по мере "
    ...
    P("Эмпирические зафиксированные прогоны показывают, что адаптивное управление может превосходить "
      "неадаптивные базисы: в эксперименте E совместная политика точнее каждого фиксированного "
      "порога на +4…+8 п.п. и быстрее примерно в 3,7 раза. Эти числа используются как "
      "thesis-snapshot evidence и мотивация; новые post-defense block-v1 benchmark claims требуют rerun. "
      "Научная новизна работы состоит в первой формализации совместного управления длиной "
      "черновика и порогом верификации как единого марковского процесса принятия решений с "
      "доказуемыми свойствами; ранее эти две оси рассматривались в литературе порознь."),
...
APP_A = block([
    ...
    H2("А.3 Теорема B (исправленная reward-shaping формулировка)"),
...
APP_B = block([
    ...
    LISTING("Листинг Б.3 — Шаг онлайн-декодера (jointadaspec/inference/jointadaspec.py)",
            ...
            "    self._verify_block(...)            # нечёткая верификация\n"
            "else:\n"
            "    k = min(k + 1, self.policy.config.gamma_max)   # продолжить\n"
            "self.K_prev = kl_divergence(q_probs, p_probs)  # задержка K"),
])
