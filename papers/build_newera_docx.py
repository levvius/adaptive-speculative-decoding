...
REFERAT = block([
    ...
    P("Основные результаты. Предложен метод JointAdaSpec, формализующий совместное "
      ...
      "→ Qwen2.5-0.5B-Instruct трактуется как legacy / pre-repair evidence: он показывает "
      ...
      "финальное подтверждение block-v1 требует повторного прогона с новой 9-action "
...
RAZDEL2 = block([
    ...
    P("Действие на каждом шаге выбирается из пространства версии 2: либо продолжить "
      ...
      "порогов T. Старое имя stop сохраняется только как legacy-алиас входных артефактов "
...
def razdel4(figs):
    ...
    H2("4.3 Legacy evidence: пара 14B/0.5B"),
    P("На основной паре сравнивались четыре метода: прямая генерация целевой моделью "
      ...
      "таблице 4.2 как legacy / pre-repair evidence: они не являются финальным block-v1 "
      "claim без повторного прогона с новой 9-action policy."),
    TBL_CAPTION("Таблица 4.2 — Legacy evidence на паре 14B/0.5B (GSM8K, n = 1500)"),
    ...
    P("В pre-repair артефактах совместная политика повышает точность на +4,07 п.п. "
      ...
      "мотивационное legacy evidence, а не как финальное доказательство block-v1. "
      ...
      "реализация и воспроизводимый план rerun отделены от legacy-чисел»."),
    P("Парный характер критерия Макнемара здесь существен. Значимость определяют не валовые "
      ...
      "расходятся. В pre-repair артефактах этот перевес давал p < 0,05 и доверительный "
      "интервал [+0,73; +7,40], но после аудита этот вывод используется только как "
      "историческое свидетельство; финальный block-v1 benchmark claim требует fresh rerun."),
    figs["pareto"],
    ...
    P("Совокупность результатов складывается в следующий честный вывод. Вклад работы — это "
      ...
      "«совместная политика побеждает все методы». Pre-repair прогоны показывают потенциал "
      "адаптивного управления относительно ванильного SD и фиксированного нечёткого порога, "
      "но финальные block-v1 claims требуют повторного прогона. Совместная политика "
...
ZAKL = block([
    ...
    P("Предложенный метод JointAdaSpec формализует выбор пары (длина черновика, порог "
      ...
      "Qwen2.5-14B → 0.5B с высоким отношением мощностей legacy-прогон показывает потенциал "
      ...
      "финального block-v1 claim, который требует повторного прогона с новой политикой."),
    ...
    P("Эмпирические legacy-прогоны показывают, что адаптивное управление может превосходить "
      ...
      "pre-repair evidence и мотивация; финальные block-v1 benchmark claims требуют rerun. "
