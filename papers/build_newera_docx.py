...
REFERAT = block([
    H1("Реферат"),
    P("Выпускная квалификационная работа (магистерская диссертация): 88 с., "
      "4 раздела, 12 рис., 10 табл., 19 источников, 4 приложения.", indent=False, jc="both"),
...
SODER = block([
    ...
    TOC_LINE("1.2.3 Вероятность принятия и формула ускорения", 17, level=2),
    ...
    TOC_LINE("1.3.3 Ограниченность фиксированных гиперпараметров", 20, level=2),
    ...
    TOC_LINE("1.4.5 Ослабленная адаптивная верификация и фронт Парето", 23, level=2),
    ...
    TOC_LINE("1.5.2 Метод value iteration: сходимость и вычислительная сложность", 26, level=2),
    TOC_LINE("1.5.3 Монотонные и пороговые политики", 27, level=2),
    TOC_LINE("1.6 Исследовательский пробел и постановка задачи", 27, level=1),
    TOC_LINE("1.6.1 Ограничения существующих адаптивных методов", 27, level=2),
    TOC_LINE("1.6.2 Идея совместного адаптивного управления", 28, level=2),
    TOC_LINE("1.6.3 Формальная постановка задачи совместной оптимизации", 28, level=2),
    TOC_LINE("2 Метод JointAdaSpec и его теоретический анализ", 30, bold=True),
    TOC_LINE("2.1 MDP-формулировка совместной оптимизации", 30, level=1),
    TOC_LINE("2.1.1 Пространство состояний и его дискретизация", 30, level=2),
    TOC_LINE("2.1.2 Пространство действий", 32, level=2),
    TOC_LINE("2.1.3 Функция переходов и функция награды", 33, level=2),
    TOC_LINE("2.2 Теоретический анализ метода", 35, level=1),
    TOC_LINE("2.2.1 Выборочная сложность оценщика переходов (теорема A)", 35, level=2),
    TOC_LINE("2.2.2 Беллман-инвариантность аддитивного штрафа качества (теорема B)", 36, level=2),
    TOC_LINE("2.2.3 Субоптимальность каскадных политик (теорема C)", 37, level=2),
    TOC_LINE("2.2.4 Точный value gap совместной и каскадной политик (теорема D)", 38, level=2),
    TOC_LINE("2.2.5 Скаляризация Парето-фронта (теорема 2.4)", 39, level=2),
    TOC_LINE("2.3 Алгоритм решения MDP", 40, level=1),
    TOC_LINE("2.4 Инференс-алгоритм с выученной политикой", 41, level=1),
    TOC_LINE("3 Программная реализация", 43, bold=True),
    TOC_LINE("3.1 Архитектура программного комплекса", 43, level=1),
    TOC_LINE("3.2 Модуль сбора трейсов", 45, level=1),
    TOC_LINE("3.3 Модуль оценки MDP и решения value iteration", 46, level=1),
    TOC_LINE("3.4 Модуль инференса с политикой JointAdaSpec", 48, level=1),
    TOC_LINE("3.5 Реализация baseline-методов", 49, level=1),
    TOC_LINE("3.6 Инструменты, тестирование и воспроизводимость", 50, level=1),
    TOC_LINE("4 Экспериментальное исследование", 52, bold=True),
    TOC_LINE("4.1 Методика экспериментов и метрики", 53, level=1),
    TOC_LINE("4.2 Исследуемые пары моделей и наборы данных", 54, level=1),
    TOC_LINE("4.3 Основной результат: пара 14B/0.5B", 55, level=1),
    TOC_LINE("4.4 Нуль-результат и триангуляция: пара 7B/1.5B", 56, level=1),
    TOC_LINE("4.5 Адаптивность против фиксированного порога (теорема E)", 58, level=1),
    TOC_LINE("4.6 Анализ компромисса по κ и нелинейности качества (теорема G)", 59, level=1),
    TOC_LINE("4.7 Сравнение с AutoJudge и обсуждение результатов", 60, level=1),
    TOC_LINE("ЗАКЛЮЧЕНИЕ", 63, bold=True),
    TOC_LINE("СПИСОК ИСПОЛЬЗОВАННЫХ ИСТОЧНИКОВ", 66, bold=True),
    TOC_LINE("ПРИЛОЖЕНИЕ А. Доказательства теорем", 69, bold=True),
    TOC_LINE("ПРИЛОЖЕНИЕ Б. Архитектура репозитория и листинги", 79, bold=True),
    TOC_LINE("ПРИЛОЖЕНИЕ В. Дополнительные экспериментальные данные", 83, bold=True),
    TOC_LINE("ПРИЛОЖЕНИЕ Г. Манифесты воспроизводимости", 87, bold=True),
])
...
# --------------------- Приложения ---------------------
def APP(letter, title, note):
    return block([H1(f"Приложение {letter}. {title}"), _plan(note)])

APP_A = APP("А", "Доказательства теорем",
            "Полные доказательства теорем A, B, C, D, 2.3, 2.4 и G; оформляется вместе "
            "со вторым разделом.")
APP_B = APP("Б", "Архитектура репозитория и листинги",
            "Схема зависимостей модулей jointadaspec/ и листинги ключевых алгоритмов "
            "(оценка MDP, итерация по ценности, инференс-декодер).")
APP_V = APP("В", "Дополнительные экспериментальные данные",
            "Парето-фронты, пороговые поверхности, графики условий C1–C4 / N1–N2 и "
            "развёртки по κ из каталога reports/thesis_figs/.")
APP_G = APP("Г", "Манифесты воспроизводимости",
            "Манифесты прогонов с git SHA, флагом dirty, разрешённым YAML-конфигом, "
            "списком начальных значений и SHA256 артефактов трейсов и политики.")
