...
This is a historical repository-stabilization note. The defense-facing entry
points are `README.MD`, `docs/RESULTS.md`,
`docs/DEFENSE_AUDIT_2026-06-26.md`, and
`docs/COMMISSION_QA_2026-06-26.md`.

## Summary

Goal: align the code, experiments, README/docs, thesis, and defense materials around a coherent thesis snapshot: block speculative decoding, a matching MDP formulation, honest statistical reporting, reproducible artifacts, and GitHub publication.

Current archived thesis snapshot Git state must be checked before every commit. At the time this plan was created, the worktree already contained a modified `papers/ИС61_fpm_КозинАА_2026.docx` and an untracked `AGENTS.md`. Those changes must not be mixed into code commits unless they are explicitly staged as part of a documentation commit.

## Key Changes

1. Frame archived JointAdaSpec results as thesis-snapshot evidence and route new block-v1 claims to main.
...
Use a separate stabilization branch:

```bash
git switch -c codex/stabilize-repo-thesis
```

Logical commits:

1. `docs: add repository and thesis maintenance notes`
2. `docs: mark thesis-snapshot results and update project guidance`
...
```bash
git push -u origin codex/stabilize-repo-thesis
```

Then open a PR from the stabilization branch into `main`. Do not push directly to `main`.

## Documentation Updates

- README: describe the snapshot/future-research boundary.
...
These items must be handled before treating the stabilization branch as merge-ready. They cover compatibility, validation, and documentation risks discovered after the first stabilization commits.
...
- Action-space migration: version policy and trace artifacts with `action_space_version=2` and `decoder_semantics=block_verify_v1`. Archived 16-action policy `.npz` artifacts must stay attached to their snapshot; loaders should reject metadata-less artifacts or mark them explicitly as archived.
...
- Remaining claim cleanup: add an `rg` gate for stale phrases and fix remaining conflicts in `reports/dissertation_review_2026-05-15.md` and `tests/test_mdp_solver.py`, especially old Bellman-invariance wording around Theorem B.
- Schema/report compatibility: document schema v3, ensure compatibility readers and report templates either read v3 or fail with a clear compatibility message, and add paired prompt-clustered comparisons with Holm correction for multiple comparisons.
...
- MacBook-safe: `git diff --check`, `python3 -m py_compile` for changed Python files, `rg` checks for banned or snapshot claims, and schema validation on a small synthetic JSONL.
...
- Work happens on a dedicated stabilization branch.
- GitHub remote is `origin`.
- Push goes to the stabilization branch, not directly to `main`.
- Old results remain preserved as artifact-backed thesis evidence but are not a new post-defense claim for block-SD speedup.
- New post-defense benchmark claims depend on rerunning the benchmark after block verification is implemented.
